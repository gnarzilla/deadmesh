...
<img src="https://raw.githubusercontent.com/gnarzilla/deadmesh/main/src/assets/media/deadmesh_ui_v1.1.2.gif" width="540" alt="UI 1.1.2">
...
The key is treating LoRa as a **dumb byte pipe** and putting all the intelligence in the proxy layer above it. Everything that speaks HTTP already works, without modification. Every protocol deadmesh adds benefits every application simultaneously.
...
# Build
make -f Makefile.macports UI=1

# Generate CA
mkdir -p ~/.deadlight
openssl genrsa -out ~/.deadlight/ca.key 4096
openssl req -new -x509 -days 3650 -key ~/.deadlight/ca.key \
  -out ~/.deadlight/ca.crt -subj "/CN=deadmesh CA"
